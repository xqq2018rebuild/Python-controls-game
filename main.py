...
def get_screen(photo_name):
    os.system("ld /system/bin/screencap -p /storage/sdcard0/Pictures/%s " % (photo_name))
    pwd=os.getcwd()
    shutil.move(os.path.join("C:\\Users\\lz977\\Documents\\雷电模拟器\\Pictures\\",photo_name),os.path.join(pwd,"tmp\\tmp"))
...
def capture(filename):
    ...
    for x in range(10):
        im = ImageGrab.grabclipboard()  # 从剪切板读图
...
def window_capture(filename):
    ...
    hwnd = win32gui.FindWindow(None,u"雷电模拟器")
...
def see_to_tap2(im_source, im_search,n=0,ranx=50,rany=15):
    """
    找到什么点什么
    :param im_source: 已经使用cv2.imread(filename) 打开过的图片
    :param im_search: 已经打开的位图块
    ...
    L = find_all_template(im_source, im_search)
    if len(L) >= 1:
        tap(L[n]["result"][0] + randx, L[n]["result"][1] + randy)
        print("find something...")
        return 1
    return 0
...
def hunshi2():
    ...
    while times <=100:
        ...
        imnow = imread("tmp/tmp/now%s.png"%(beg))
        # 点击挑战
        if see_to_tap2(imnow,imtiaozhan):
            times += 1
            print("目前第%s次挑战" % (times))
        # 点击胜利
        see_to_tap2(imnow,imshengli2)
        see_to_tap2(imnow,imjieshou)
...
#     tap(145,335)
#     # time.sleep(2)
#     # swipe(848,588,748,570)  #选狗粮
#     time.sleep(4)
#     swipe(329,571,215,317)
#     time.sleep(4)
#     swipe(597,578,635,368)
#     time.sleep(2)
#     tap(1170,543) #准备
